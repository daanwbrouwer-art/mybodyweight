...
export default function OnboardingPage() {
  const [step, setStep] = useState<"gender" | "level">("gender");
  ...
  const handleGenderSelect = (g: string) => setGender(g);

  const handleGenderContinue = () => {
    if (gender) setStep("level");
  };

  const handleLevelTap = (lvl: (typeof LEVELS)[number]) => {
    if (lvl.locked) {
          ...
          {step === "level" && (
            ...
            >
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-sm">
                  Start free. Unlock more as you progress.
                </p>
              </div>
              ...
              <div className="w-full flex flex-col gap-3">
                {LEVELS.map((lvl) => (
                  <button
                    ...
                    className={`w-full rounded-2xl p-5 border transition-all duration-300 relative overflow-hidden text-left ${
                      lvl.locked
                  ...
                  >
                    {/* Gradient shimmer for locked */}
                    {lvl.locked && (
                      <div
                        className="absolute inset-0 pointer-events-none"
                        style={{
                          background:
                            "linear-gradient(135deg, transparent 0%, oklch(0.83 0.19 84 / 0.03) 100%)",
                        }}
                      />
                    )}
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`text-3xl w-12 h-12 flex items-center justify-center rounded-xl ${
                          lvl.locked ? "bg-muted/60" : "bg-primary/15"
                        }`}
                      >
                        {lvl.locked ? (
                          <svg
                      ...
                      {!lvl.locked && selectedLevel === lvl.id && (
              ...
              <AnimatePresence>
                {showUpgradeHint && (
                  ...
                  >
                    <span className="font-semibold">Upgrade to Subscriber</span>{" "}
